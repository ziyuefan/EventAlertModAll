...
function EventAlert_OnLoad(self)
	...
	--	["UNIT_COMBO_POINTS"]			= EventAlert_COMBO_POINTS,
		...
		["UPDATE_UI_WIDGET"]			= EventAlert_UPDATE_UI_WIDGET,
	}
...
--[[------------------------------------------------------------------
--------------------------------------------------------------------]]
function EventAlert_UPDATE_UI_WIDGET(self,event,...)
		local UIWidgetInfo = ...
		...
		if UIWidgetInfo.widgetType == 24 then 
			...
			local vitality 		= numFullFrames 	* fillMax + fillValue
			local vitalityMax 	= numTotalFrames	* fillMax 
			print(format("總活力值%d:%d", DragonEnergyMax, DragonEnergy))
		end
end
--[[------------------------------------------------------------------
--------------------------------------------------------------------]]
	...
	if (EA_Config.ShowFrame == true) then
		...
		for k,v in ipairs(EA_TarCurrentBuffs) do
			...
			if eaf ~= nil then
				...
				Lib_ZYF:FrameSetOnUpdate(eaf, EventAlert_UpdateInterval, EventAlert_OnTarUpdate, spellID)
				-- eaf:SetScript("OnUpdate", function(self,elapsedTime)
